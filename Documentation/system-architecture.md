# FTRFS System Architecture

## Positioning

FTRFS is not a general-purpose filesystem and does not attempt to replace
...
Understanding where FTRFS fits requires understanding what the other
...
FTRFS provides **in-place error correction** for **read-write** partitions.
...
## Why dm-verity and FTRFS are complementary, not competing

dm-verity and FTRFS operate on different partition types and address
different failure modes:

| Property              | dm-verity + squashfs   | FTRFS                    |
|-----------------------|------------------------|--------------------------|
...
data partition. A system that uses only FTRFS on the root filesystem
...
of FTRFS within a complete hardened embedded Linux system:
...
This layout is not theoretical. The FTRFS Yocto layer
(`github.com/roastercode/yocto-hardened`, branch `arm64-ftrfs`) deploys
FTRFS as a dedicated data partition (`/dev/vdb`) in an arm64 Slurm HPC
...
FTRFS addresses the case where hardware-level radiation hardening is
...
FTRFS is not an RTOS and does not compete with PikeOS. On a Linux
partition within a PikeOS mixed-criticality system, FTRFS could serve
...
FTRFS in its current version is validated as a **data partition**
...
The current FTRFS integrity model uses CRC32 (per-block, per-inode,
...
This extension is out of scope for the current FTRFS design, which
