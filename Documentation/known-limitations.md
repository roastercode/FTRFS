...
| 6.3 Superblock RS correction | Implemented in stage 3 item 2 (v0.3.0+). The superblock is protected by both CRC32 (detection) and RS(255,239) shortened over 8 sub-blocks of 211 data bytes (correction). On mount, a CRC32 mismatch triggers `ftrfs_rs_decode_region()` over the staging buffer, with parity at offset 3968 of the superblock block. Recovery succeeds on up to 8 byte errors per sub-block. The corrected superblock is persisted to disk on the next metadata mutation via `ftrfs_dirty_super()`, which encodes RS parity then recomputes CRC32 in that order. See `Documentation/design.md`, sections "Superblock CRC32" and "Superblock RS FEC". |
