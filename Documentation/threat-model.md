# FTRFS Threat Model and Market Positioning

**Status**: design document, normative for FTRFS architectural decisions.
...
This document defines the failure modes FTRFS is designed to mitigate,
...
FTRFS protects against two distinct families of failure that share a
...
2. **Universal coverage**, not opt-in. An adversary will not
   ...
   All data blocks in a mounted FTRFS volume must be protected by
   default.
...
### 2.3 What FTRFS explicitly does not address
...
- **Confidentiality.** Disk encryption (dm-crypt, LUKS) is
  orthogonal and recommended below FTRFS where required.
...
- **Fault tolerance against device-level catastrophic failure**
  (chip burn-out, controller failure). FTRFS operates within a
...
- **Knowledge**: assumed to know the FTRFS on-disk format, the
  RS parameters, and the mount-time recovery procedure. Security
  through obscurity is rejected.

For Family A, there is no actor; the model is purely physical. FTRFS
...
This is Family A territory at scale. FTRFS provides the missing
filesystem layer.
...
This is Family B territory. FTRFS protects the integrity of the
...
The following table maps FTRFS's threat model against the protections
...
The single combination of capabilities required by the FTRFS threat
...
**FTRFS occupies this specific gap by design.** It does not
...
## 6. Implications for FTRFS architecture
...
must apply to all data blocks of a mounted FTRFS volume by
default. A per-inode opt-in flag (such as a hypothetical
`FTRFS_INODE_FL_RS_ENABLED` controlling data block protection) is
rejected.
...
and the field already exists in the on-disk `ftrfs_rs_event` reserved
...
tools complementing FTRFS, not replacing it.

---

## 7. Relationship with the certification regimes

| Regime           | Domain               | Code auditability requirement | FTRFS posture |
|------------------|----------------------|-------------------------------|---------------|
...
FTRFS targets being **suitable for inclusion** in systems undergoing
...
- Fuchs, Langer, Trinitis. *FTRFS: A Fault-Tolerant Radiation-Robust Filesystem for Space Use.* ARCS 2015, LNCS 9017. https://www.cfuchs.net/chris/publication-list/ARCS2015/FTRFS.pdf
